...
# Utils
from utils import DATE_FORMAT, TODAY, string_to_float, es_casi_igual, get_dollars
...
class DetailWin:
  ...
  def __init__(self, sale_id, callback_functions, params):
    self.params = params
    ...
    self.sale_date_entry = ttk.Entry(
        frame,
        width=10,
        font=('calibri', 15))
    ...
    self.payments_handler.display_payments_tree(payments_frame, True, callbacks=[self.calculate_remaining])
    ...
    add_product_button = tk.Button(
        ...
        relief=tk.RIDGE,
        bg='#54bf54',
        command=lambda: self.products_handler.display_new_order_window(self.rate.get()))
    ...
    add_payment_button = tk.Button(
        ...
        relief=tk.RIDGE,
        bg='#54bf54',
        command=lambda: self.payments_handler.add_payment_window(self.query_date.get(), self.rate.get(), False, True))
